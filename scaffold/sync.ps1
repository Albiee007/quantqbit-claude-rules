...
# === harness sync (PowerShell entry point) ===
# Thin wrapper: runs sync.sh with Git Bash so Windows, macOS and Linux share one
# implementation. Git for Windows' bash is preferred over WSL's bash.exe, which
# cannot resolve Windows paths.
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File scaffold\sync.ps1 [options]
#   (same options as sync.sh; run with --help to list them)
#
...
if (-not $bash) {
    ...
    Write-Host "       Or run sync.sh from WSL, macOS or Linux."
    exit 2
}
